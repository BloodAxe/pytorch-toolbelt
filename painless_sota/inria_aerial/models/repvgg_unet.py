...
from pytorch_toolbelt.utils import describe_outputs, count_parameters
from timm.models.layers import DropPath

__all__ = ["RepVGGUnet", "RepVGGBlock"]
...
class RepVGGBlock(nn.Module):
    def __init__(
        ...
        self.nonlinearity = instantiate_activation_block(activation, inplace=True)
    ...
    def forward(self, inputs):
        ...
        conv3x3_path = self.drop_path(self.se(self.rbr_dense(inputs)))
        conv1x1_path = self.drop_path(self.rbr_1x1(inputs))
        return self.nonlinearity(conv3x3_path + conv1x1_path + id_out)
...
def create_RepVGG_A0(**kwargs):
    return RepVGG(
        num_blocks=[2, 4, 14, 1], width_multiplier=[0.75, 0.75, 0.75, 2.5], override_groups_map=None, **kwargs
    )


def create_RepVGG_A1(**kwargs):
    return RepVGG(num_blocks=[2, 4, 14, 1], width_multiplier=[1, 1, 1, 2.5], override_groups_map=None, **kwargs)


def create_RepVGG_A2(**kwargs):
    return RepVGG(
        num_blocks=[2, 4, 14, 1],
...
class RepVGGUnet(nn.Module):
    ...
    def __init__(
        ...
        self.decoder = UNetDecoder(
            ...
            decoder_features,
            unet_block=self.unet_decoder_block,
    ...
    def forward(self, image):
        ...
        segmentation = self.segmentation_head(feature_maps, output_size)
        return segmentation

    def unet_decoder_block(self, input_channels, output_channels):
        return nn.Sequential(
            RepVGGBlock(
...
if __name__ == "__main__":
    ...
    outputs = model(input)
    pprint(describe_outputs(outputs))
    pprint(count_parameters(model, human_friendly=True))

...
__all__ = ["FPNSumDecoder", "FPNSumTransitionBlock", "FPNSumCenterBlock"]
...
class FPNSumCenterBlock(nn.Module):
    def __init__(
        ...
        self.bottleneck = nn.Conv2d(
            encoder_features, encoder_features // 2, kernel_size=1
        )

        self.pool2 = nn.AvgPool2d(kernel_size=2, stride=2)
        self.proj2 = nn.Conv2d(
            encoder_features // 2, encoder_features // 8, kernel_size=1
        )
        ...
        self.blend = nn.Conv2d(
            encoder_features // 2 + 3 * encoder_features // 8,
            decoder_features,
            kernel_size=1,
        )
        ...
        self.conv1 = nn.Conv2d(
            decoder_features, decoder_features, kernel_size=3, padding=1, bias=False
        )
...
class FPNSumTransitionBlock(nn.Module):
    def __init__(
        self,
        encoder_features: int,
        decoder_features: int,
    ...
    def forward(self, decoder_fm: Tensor, encoder_fm: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        :return:
        """
        decoder_fm = F.interpolate(
            decoder_fm, size=encoder_fm.size()[2:], mode="bilinear", align_corners=True
        )
...
class FPNSumDecoder(DecoderModule):
    """
    ...
    def __init__(
        ...
        self.fpn_modules = nn.ModuleList(
            [
                ...
                for decoder_fm, encoder_fm in zip(
                    repeat(fpn_channels), reversed(feature_maps[:-1])
                )
            ]
        )
